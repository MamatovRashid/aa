<template>
  <div class="section__list attestation flex-column">
    ...
    <div class="work-time__filter mt-5">
      <div class="work-time__filter__menu">
        <div class="filter__wrapper" v-if="!hideFilter">
          ...
          <button class="button primary-btn" @click.prevent="clearForm">
            Очистить
          </button>
        </div>
      </div>
    ...
    <d-lists class="flex-1" :loading="loading" dashed-header>
      <template #header>
        <div class="columns mx-0">
          <div class="column is-8"><span>Сотрудник</span></div>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      this.loading = true;
      let params = {}
      this.from && (params.date_from = this.from)
      this.to && (params.date_to = this.to)
      if(this.from && !this.to) {
        // params.date_to = (new Date()).split("T")[0]
        let day = new Date()
        let returnVal = day.getFullYear() + "-" + (day.getMonth() + 1) + "-" + day.getDate()
        params.date_to = returnVal
      }
